...
function toCSV(data: MapPremise[]) {
  const headers = [
    "cid",
    "placeId",
    "name",
...
function extractPremise(card: Element): MapPremise | null {
  ...
  const mainLink = Array.from(card.querySelectorAll("a"))
    .find(a =>
      (a as HTMLAnchorElement).href?.includes("/maps/place/")
  ...
  // ─────────────────────────────────────────────
  const name =
    mainLink?.getAttribute("aria-label") ||
    (card.querySelector('[aria-label]') as HTMLElement | null)
  ...
  const isGarbageText = (t: string) => {
    if (!t) return true;

    const cleaned = t.trim();

    return (
      cleaned.length <= 1 ||                 // single glyph like 
  ...
  const statusIdx = spans.findIndex(t => {
    ...
    return (
      /^open(\s24\s?hours)?/.test(s) ||   // Open / Open 24 hours
  ...
  for (const div of allDivs) {
    ...
    const isDefaultAvatar =
      img?.src?.includes("default_user.png");

    if (!isDefaultAvatar) continue;
...
function scrapeFeed(): MapPremise[] {
  const feed = document.querySelector('[role="feed"]');
  if (!feed) return [];

  const cards = feed.querySelectorAll('[role="article"]');

  const results: MapPremise[] = [];
...
let EXTRACTED_FEEDS: MapPremise[] = [];
...
function attachEvents() {
  ...
  btnExtractFeed?.addEventListener("click", async () => {
    ...
    EXTRACTED_FEEDS = data.filter((item) => {
      ...
      if (!key) return true; // keep if no identifier exists
  ...
  btnDownload?.addEventListener("click", () => {
    ...
    if (!EXTRACTED_FEEDS.length) return alert("No data to download");

    const blob = new Blob(
      [JSON.stringify(EXTRACTED_FEEDS, null, 2)],
